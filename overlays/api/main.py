...
if settings.MODE != "local" and not settings.STATIC_BEARER_TOKEN:
    try:
        ...
        logger.warning("slowapi is not installed in the base API image; rate limiting disabled")
else:
    logger.info("Rate limiting disabled for local/static-token deployment")
