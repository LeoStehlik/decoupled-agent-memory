...
- Review queues can grow stale if no one owns the final update.

## Required Control

The maintenance layer must report stale synthesis and duplicate active paths before the system claims to be healthy. The review layer must then show which synthesis page needs attention, which source changed, and enough evidence for an agent or human to update the maintained page deliberately.
