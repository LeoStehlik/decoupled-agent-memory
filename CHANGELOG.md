...
- Added apply-proof metadata for accepted review decisions, including stale-before/stale-after counts, target path, linked source count, proposal hash, and page-clean status.
- Added review decision detail and revert-suggestion API endpoints.
- Added MCP `review_decision_detail` and `revert_suggestion` tools.
...
- Changed the Artifacts tab to show post-apply proof and expandable decision details with stored diff/proof metadata.
